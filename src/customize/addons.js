...
const InputLineComponent = () => {
    ...
    const _addEventListiner = (current) => {
        ...
        checkboxes.forEach((e) =>
            e.addEventListener('click', _onCheckboxClick)
        );
    ...
    const _updateDirection = (current) => {
        ...
        const { value } =
            current.parentElement.nextElementSibling.querySelector('input');
        const checkboxes = current.querySelectorAll("input[type='checkbox']");
        checkboxes.forEach(
            (e) => (e.checked = new RegExp(e.name, 'i').test(value))
        );
        current.closest(
            '[kc-panel-setting]'
    ...
    const init = (current) => {
        ...
        htmlObj.innerHTML =
            `<div>${checkboxList}</div><div class="Polaris-Stack">${radioList}</div>`.replace(
                /,/g,
                ''
            );
...
({
    ...
    toggleTag(value, { current }) {
        ...
        _TEST.target.current.parentElement.nextElementSibling.querySelector(
            'input'
        ).value;
